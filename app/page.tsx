...
import { Calendar, CheckCircle2, Cloud, DollarSign, FileText, Filter, Inbox, Layers, Loader2, Mail, MessageSquare, PanelLeft, Plus, Search, Server, Settings, ShieldCheck, Slack, Upload, Users, Youtube, ArrowUpRight, AlertTriangle, ChevronRight, Folder, Bell } from 'lucide-react';
    ...
    <div className="min-h-screen bg-neutral-50 text-neutral-900">
      ...
      <div className="mx-auto max-w-7xl p-4 grid grid-cols-12 gap-4">
        <aside className="col-span-3 xl:col-span-2 space-y-2">
          {NavItem({icon:<Inbox className='h-4 w-4'/>, label:'Dashboard', active:true})}
          {NavItem({icon:<Layers className='h-4 w-4'/>, label:'Tasks'})}
          {NavItem({icon:<CheckCircle2 className='h-4 w-4'/>, label:'Quality Control'})}
          {NavItem({icon:<Calendar className='h-4 w-4'/>, label:'Scheduling'})}
          {NavItem({icon:<Mail className='h-4 w-4'/>, label:'Client Delivery'})}
          {NavItem({icon:<Users className='h-4 w-4'/>, label:'Team'})}
          {NavItem({icon:<DollarSign className='h-4 w-4'/>, label:'Payments'})}
          {NavItem({icon:<FileText className='h-4 w-4'/>, label:'Documents'})}
          {NavItem({icon:<ShieldCheck className='h-4 w-4'/>, label:'Audit Logs'})}
                    ...
                    <TableBody>
                      {tasks.map((t) => (
                        <TableRow key={t.id}>
                          ...
                          <TableCell>
                            <Badge className={t.priority === 'High' ? 'bg-red-500' : t.priority === 'Medium' ? 'bg-amber-500' : 'bg-neutral-500'}>{t.priority}</Badge>
                          </TableCell>
                          ...
                          <TableCell className="text-right">
                            <Dialog>
                              <DialogContent className="max-w-2xl">
                                <DialogHeader>
                                  <DialogTitle>{t.id}: {t.title}</DialogTitle>
                                </DialogHeader>
                                <div className="grid md:grid-cols-2 gap-4 mt-2">
                                  <div className="space-y-2">
                                    {LabelRow({label:'Assignee', value:t.assignee})}
                                    {LabelRow({label:'Client', value:t.client})}
                                    {LabelRow({label:'Stage', value:t.stage})}
                                    {LabelRow({label:'Due', value:t.due})}
                                  </div>
                                  <div className="space-y-2">
...
function NavItem({ icon, label, active }: { icon: React.ReactNode; label: string; active?: boolean }) {
  return (
    <button className={`w-full flex items-center gap-3 px-3 py-2 rounded-xl border ${active ? 'bg-neutral-900 text-white border-neutral-900' : 'bg-white hover:bg-neutral-50 border-neutral-200'}`}>
